...
export default function VisitorBadge({ className = '', initiallyExpanded = false }: { className?: string; initiallyExpanded?: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex min-h-9 w-full items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-left text-xs text-slate-600 shadow-sm dark:border-neutral-700 dark:bg-[#141413] dark:text-slate-300 sm:w-auto sm:justify-start ${className}`}
      aria-expanded="true"
